...
function Shell() {
  ...
  return (
    <Frame nav={<BottomNav active={tab} onSelect={setTab} />} screen={tab}>
      ...
      {tab === 'einstellungen' && <SettingsScreen onNavigate={setTab} />}
      {tab === 'eigene' && <CustomWordsScreen onNavigate={setTab} />}
    </Frame>
  );
}

/** Full-screen on phones; a centered, phone-sized "device" window on larger screens. */
function Frame({ children, nav, screen }: { children: ReactNode; nav?: ReactNode; screen?: string }) {
  const main = useRef<HTMLElement>(null);
  // The screens share one scroll container: a new screen must start at the top,
  // not where the previous one was scrolled to.
  useEffect(() => {
